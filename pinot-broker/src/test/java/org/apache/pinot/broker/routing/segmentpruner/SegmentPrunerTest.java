...
public class SegmentPrunerTest {
  ...
  private static final String QUERY_5 = "SELECT * FROM testTable where time = 40";
  private static final String QUERY_6 = "SELECT * FROM testTable where time BETWEEN 20 AND 30";
  private static final String QUERY_7 = "SELECT * FROM testTable where 30 < time AND time <= 50";
  private static final String QUERY_8 = "SELECT * FROM testTable where time < 15 OR time > 45";
  private static final String QUERY_9 = "SELECT * FROM testTable where time < 15 OR (60 < time AND time < 70)";
  private static final String QUERY_10 = "SELECT * FROM testTable where time < 0 AND time > 0";
  ...
  public void testSegmentPrunerFactoryForTimeRangePruner() {
    ...
    when(routingConfig.getSegmentPrunerTypes())
        .thenReturn(Collections.singletonList(RoutingConfig.TIME_RANGE_SEGMENT_PRUNER_TYPE));
    assertEquals(SegmentPrunerFactory.getSegmentPruners(tableConfig, _propertyStore), Collections.emptySet());
    ...
    List<SegmentPruner> segmentPruners = SegmentPrunerFactory.getSegmentPruners(tableConfig, _propertyStore);
    assertEquals(segmentPruners.size(), 1);
    assertTrue(segmentPruners.get(0) instanceof TimeRangeSegmentPruner);
  ...
  public void testTimeRangeSegmentPruner() {
    ...
    TimeRangeSegmentPruner segmentPruner = new TimeRangeSegmentPruner(tableConfig, _propertyStore);
    Set<String> onlineSegments = new HashSet<>();
    segmentPruner.init(externalView, idealState, onlineSegments);
    assertEquals(segmentPruner.prune(brokerRequest1, Collections.emptySet()), Collections.emptySet());
    assertEquals(segmentPruner.prune(brokerRequest2, Collections.emptySet()), Collections.emptySet());
